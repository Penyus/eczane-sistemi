...
// Backend alan adlarını UI alan adlarına eşle
const adaptedData = {
  ...data,
  medicines: (data?.ilaclar || []).map(i => ({
    id: i.id,
    ad: i.ilac_adi,
    stok: i.stok_miktari,
  ...
  movements: (data?.hareketler || []).map(h => ({
    id: h.id,
    ilac: h.ilac_adi,
    miktar: h.islem_tipi === "giris" ? h.miktar : -h.miktar,
    tur: h.islem_tipi === "giris" ? "entry" : h.islem_tipi === "satis" ? "exit" : "exit",
    kullanici: h.kullanici?.first_name || h.kullanici?.username || "",
    tarih: h.islem_tarihi?.substring(0, 16).replace("T", " ") || "",
  })),
  prescriptions: data?.receteler || [],
  summary: {
    toplamIlac: data?.stokOzeti?.toplam_ilac || 0,
    toplamStok: (data?.ilaclar || []).reduce((s, i) => s + i.stok_miktari, 0),
    kritikSayisi: data?.stokOzeti?.kritik_stok_ilac_sayisi || 0,
    miadYaklasanSayisi: data?.stokOzeti?.miad_yaklasan_ilac_sayisi || 0,
    bugunSatis: 4280.30,
    bugunReceteSayisi: data?.stokOzeti?.bekleyen_recete_sayisi || 0,
  },
};
  const titles = {
    ...
    <div className="hk-app" data-screen-label={"Hekim · " + titles[active].title}>
      ...
      <div className="hk-main">
        ...
        <div className="hk-content">
          {active === "dashboard" && <DashboardSaas lang={lang} user={user} data={adaptedData} />}
          {active === "medicines" && <Medicines lang={lang} data={adaptedData} search={search} />}
          {active === "stock" && <StockMovements lang={lang} data={adaptedData} />}
          {active === "prescriptions" && <Prescriptions lang={lang} data={adaptedData} />}
          {active === "reports" && <Reports lang={lang} data={adaptedData} />}
          {active === "categories" && <Categories lang={lang} data={adaptedData} />}
        </div>
      </div>
    </div>
  );
